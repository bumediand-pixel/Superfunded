...
export function FeatureCard({ feature, className, ...props }: FeatureCardProps) {
  ...
  return (
    <div className={cn('relative overflow-hidden p-6 cursor-pointer group transition-all duration-300', className)} {...props}>
      ...
      <feature.icon className="w-6 h-6 text-red-500 transition-colors duration-300 group-hover:text-amber-400" strokeWidth={1.5} aria-hidden />
      <h3 className="mt-8 text-sm md:text-base font-semibold text-white/90">{feature.title}</h3>
      <p className="text-white/40 relative z-20 mt-2 text-xs font-light leading-relaxed">{feature.description}</p>
    </div>
  );
}
