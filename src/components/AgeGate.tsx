...
export default function AgeGate() {
  ...
  useEffect(() => {
    ...
    // Re-prompt on every login: listen for our custom logout event and the
    // Supabase auth event that some pages dispatch. We can't import the
    // Supabase client here (this is in the root layout, must stay light),
    // so we listen for a generic `sf-auth` CustomEvent that auth flows fire.
    const onAuth = (e: Event) => {
      const detail = (e as CustomEvent<{ event?: string }>).detail;
      if (detail?.event === 'SIGNED_IN') {
        // Force re-prompt — fresh login session.
        window.localStorage.removeItem(STORAGE_KEY);
        window.localStorage.removeItem(LEGACY_KEY);
        setVisible(true);
      } else if (detail?.event === 'SIGNED_OUT') {
        // Clear both stores so the next visitor sees the gate.
        window.localStorage.removeItem(STORAGE_KEY);
        window.localStorage.removeItem(LEGACY_KEY);
  ...
  const confirm = (over18: boolean) => {
    if (!over18) {
      // Send under-18 users to the responsible-gambling resource page.
    ...
    >
      ...
      >
        ...
        <p className="text-sm leading-relaxed mb-6" style={{ color: 'var(--text-muted)' }}>
          ...
          legat de pariuri sportive și implică riscuri financiare reale. Joacă responsabil — vezi
