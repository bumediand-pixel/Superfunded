...
// REPLACE: TOTAL_PAID_RON — pull from /api/public-stats once content is ready.
const TOTAL_PAID_RON = '€1.2M+'; // REPLACE: TOTAL_PAID_RON

export default function Hero() {
  return (
    <header className="relative overflow-hidden border-b border-[color:var(--color-ink-800)]">
      {/* Decorative radial glow — gold + volt accents */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0"
        style={{
          background:
            'radial-gradient(circle at 18% 12%, rgba(247,201,72,0.10) 0%, transparent 55%), radial-gradient(circle at 82% 88%, rgba(43,211,72,0.06) 0%, transparent 60%)',
        }}
      />

      {/* Top utility bar (logo + small auth link) */}
      <div className="relative mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
        <Link
          href="/landing"
          className="font-bebas text-2xl tracking-wider text-[color:var(--color-mist-200)]"
          aria-label="SuperFunded — acasă"
        >
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-6 pt-10 pb-20 lg:grid-cols-[1.05fr_1fr] lg:gap-16 lg:pt-16 lg:pb-28">
        ...
        <div>
          <p className="mb-5 inline-flex items-center gap-2 rounded-full border border-[color:var(--color-ink-700)] bg-[color:var(--color-ink-900)]/70 px-3 py-1.5 text-xs font-medium uppercase tracking-wider text-[color:var(--color-gold-400)]">
            <span
              aria-hidden="true"
              className="h-1.5 w-1.5 rounded-full bg-[color:var(--color-volt-400)]"
            />
          ...
          <p className="mt-6 max-w-xl text-base leading-relaxed text-[color:var(--color-mist-400)] sm:text-lg">
            Treci de provocare, primești un cont funded cu capitalul nostru și păstrezi 80% din
            profitul net — plătit săptămânal, fără să rești vreodată banii tăi.
          </p>
          ...
          <ul className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-3 text-xs text-[color:var(--color-mist-500)]">
            <li className="flex items-center gap-2">
              <ShieldCheck
                aria-hidden="true"
                className="h-4 w-4 text-[color:var(--color-volt-400)]"
              />
              <span>
                Plătit total: <strong className="text-[color:var(--color-mist-200)]">{TOTAL_PAID_RON}</strong>
              </span>
            </li>
            <li
              className="rounded border border-[color:var(--color-ink-700)] px-2 py-1 font-mono"
              aria-label="Plăți procesate prin Stripe"
            >
              {/* REPLACE: real Stripe / Visa / MC SVG logos */}
              Stripe
            </li>
            <li
              className="rounded border border-[color:var(--color-ink-700)] px-2 py-1 font-mono"
            ...
            <li className="rounded bg-[color:var(--color-ink-800)] px-2 py-1 font-bold text-[color:var(--color-gold-400)]">
              18+
            </li>
            <li className="text-[color:var(--color-mist-500)]">
              {/* REPLACE: ONJN status — licență sau notă „nu este casă de pariuri” */}
              Joacă responsabil. Nu este o casă de pariuri reglementată ONJN.
            </li>
          </ul>
        </div>

        {/* Right: hero visual — explicit dimensions for CLS=0 */}
        <div className="relative">
          <div className="relative aspect-[4/3] w-full overflow-hidden rounded-2xl border border-[color:var(--color-ink-700)] bg-[color:var(--color-ink-900)] shadow-2xl shadow-black/60">
            <Image
              ...
              alt="Captură din dashboard-ul SuperFunded: pariuri active, P/L săptămânal și progres către target"
              width={1024}
              height={768}
              priority
              fetchPriority="high"
          ...
          <div
            aria-hidden="true"
            className="absolute -bottom-6 -left-4 hidden rounded-xl border border-[color:var(--color-ink-700)] bg-[color:var(--color-ink-900)]/95 p-4 shadow-xl backdrop-blur sm:block"
          >
            <p className="text-[10px] uppercase tracking-wider text-[color:var(--color-mist-500)]">
              Profit săptămânal
            </p>
            <p className="font-mono text-2xl font-bold text-[color:var(--color-volt-400)]">
              +€842.50
            </p>
          </div>
        </div>
