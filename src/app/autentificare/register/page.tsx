      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#141414] border border-white/10 rounded-2xl p-8">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              <label className="text-white/60 text-sm mb-1.5 block">Nume complet</label>
              <input type="text" value={nume} onChange={e => setNume(e.target.value)} required
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-red-500 transition-colors" placeholder="Ion Popescu" />
            </div>
            <div>
            ...
            <div>
              <label className="text-white/60 text-sm mb-1.5 block">Parolă</label>
        ...
        <p className="text-center text-white/40 text-sm mt-6">
          Ai deja cont?{' '}
          <Link href="/autentificare/login" className="text-red-500 hover:text-red-400 font-semibold">Autentifică-te</Link>
        </p>
      </div>
    </div>
  );
